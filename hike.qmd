...
all <- (touren %>% mutate(skihalbtag = NULL, skihalbtag_fam = NULL, begleitung1 = NULL, begleitung2 = NULL, begleitung3 = NULL, 
                          begleitung4 = NULL, beg = TRUE, region = NULL, kommentar = NULL) %>% filter(sport == "Hike") %>% arrange(end, time) )

hike <- (all %>% select(start, end, date, time))
tour <- unique(hike$end)
```

# Aktuelles Jahr 

```{r, echo = FALSE, results = 'asis', message = FALSE, eval = TRUE}
t <- (touren %>% filter(sport == "Hike" & year(touren$date) == cy) %>% group_by(date) %>% arrange(desc(time), .by_group = TRUE) %>% slice(1) %>% hm_print())
```
...
```{r, echo = FALSE, results = 'asis', message = FALSE, eval = TRUE}
thm <- (touren %>% filter(sport == "Hike" & year(touren$date) == cy) %>% group_by(date) %>% arrange(desc(time), .by_group = TRUE) %>% hm_print())
