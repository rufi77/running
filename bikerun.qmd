...
# Bike and run
all <- (touren %>% mutate(skihalbtag = NULL, skihalbtag_fam = NULL, begleitung1 = NULL, begleitung2 = NULL, begleitung3 = NULL, begleitung4 = NULL, beg = NULL, region = NULL, kommentar = NULL) %>% filter(sport == "Bike and run") %>% arrange(end, time) )
...
t <- (touren %>% filter(sport == "Bike and run" & year(touren$date) == cy) %>% group_by(date) %>% arrange(desc(time), .by_group = TRUE) %>% slice(1) %>% hm_print())
```
...
```{r, echo = FALSE, results = 'asis', message = FALSE, eval = TRUE}
thm <- (touren %>% filter(sport == "Bike and run" & year(touren$date) == cy) %>% group_by(date) %>% arrange(desc(time), .by_group = TRUE) %>% hm_print())
