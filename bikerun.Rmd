...
# Bike and run
all <- (touren %>% mutate(skihalbtag = NULL, skihalbtag_fam = NULL, begleitung1 = NULL, begleitung2 = NULL, begleitung3 = NULL, region = NULL, kommentar = NULL) %>% filter(sport == "Bike and run") %>% arrange(end, time) )
...
t <- (touren %>% filter(sport == "Bike and run" & year(touren$date) == "2021") %>% group_by(date) %>% arrange(desc(time), .by_group = TRUE) %>% slice(1) %>% hm_print())
```
