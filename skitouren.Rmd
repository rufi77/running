...
t <- (touren %>% filter(sport == "Skitour" & season == "Winter 21/22") %>% hm_print())
```
...
```{r, echo = FALSE, results = 'asis', message = FALSE, eval = TRUE}
t1 <- (t %>% select(-begleitung1) %>% arrange(desc(hm_diff))) %>% rowid_to_column("Nr")
pander(t1)
```
...
```{r, echo = FALSE, results = 'asis', message = FALSE, eval = TRUE}
t1 <- (t %>% filter(begleitung1 == "Papa") %>% select(-begleitung1) %>% arrange(desc(hm_h))) %>% rowid_to_column("Nr")
pander(t1)
```
...
```{r, echo = FALSE, results = 'asis', message = FALSE, eval = TRUE}
all_m0 <- all[all$date >= "2003-12-31" & all$date <= "2022-08-31", ]
all_m <- (all_m0 %>% arrange(date) %>% group_by(date) %>% slice(1) %>% select(-season))
cuts <- parse_date_time(x = "2003-06-30", orders = "ymd") + years(x = seq.int(from = 0, to = 19, by = 1))
labs <- c(paste("Winter 0", 3:8, "/0", 4:9, sep = ""), "Winter 09/10", paste("Winter ", 10:21, "/", 11:22, sep = ""))
