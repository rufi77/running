...
plot(dat, type = "b", col = 2, xlab = "", ylab = "Anzahl Skitouren / Skihalbtage", ylim = c(0, 30))
...
t <- (touren %>% filter(is.na(hm_h) == FALSE & sport == "Skitour") %>% hm_print())
...
```{r, echo = FALSE, results = 'asis', message = FALSE, eval = TRUE}
t1 <- (t %>% select(-begleitung1) %>% arrange(desc(hm_h)))
pander(t1)
```
